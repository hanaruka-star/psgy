...
- **Kích hoạt lại Phone Auth thật cho User app** — hiện đang bypass bằng `MockPhoneAuthScreen` vì lỗi `permission-denied` từ code cũ (đọc `users/{uid}` kiểu ParkingLink, không thuộc phạm vi `pilot_demo`) khi chạy standalone với tài khoản Auth cũ còn sót trên máy test. Cần dọn/thay hẳn code đọc profile cũ đó bằng luồng `UserProfile` thật (mục 3) trước khi bật lại `PhoneAuthScreen` thật.
  ...
  **Vị trí chính xác (Cursor xác định 2026-09-04, đợt dọn ParkingLink):** `PhoneAuthScreen` watch `userProfileProvider` → `UserProfileNotifier.build()` (`lib/features/user/presentation/providers/user_profile_provider.dart`, dòng 51-66) đọc Firestore ngay nếu máy còn session Firebase — gọi `getUserProfileUseCaseProvider` (Firestore path `users/{uid}`) và `getUserVehiclesUseCaseProvider` (path `user_vehicles`, sót lại từ tính năng biển số xe ParkingLink, không có ý nghĩa gì với PSgy). Data source: `lib/features/user/data/datasources/user_profile_remote_datasource.dart` (`getProfile`/`getVehicles`). Firestore rules hiện tại là **deny-all**, nên 2 lệnh đọc này chắc chắn `permission-denied`. Khi backend thật thay `UserProfileNotifier` bằng luồng `UserProfile` mục 3, xoá luôn phần gọi `getUserVehiclesUseCaseProvider` (không có domain `Vehicle` nào ở PSgy).

  **Credential đã dọn (đợt cleanup 2026-09-04):** `macos/Runner/GoogleService-Info.plist` và `ios/Runner/Assets.xcassets/GoogleService-Info.plist` từng chứa credential của Firebase project ParkingLink cũ (`parkinglink-v2`) — đã xoá. iOS/Android hiện dùng đúng project `psgy-app` (`lib/firebase_options.dart` cũng đã đúng). Đội dev backend không cần lo phần này nữa, chỉ cần xác nhận lại 1 lần khi setup máy mới.
