                          ...
                          onSelected: (value) {
                            if (value == 'edit') {
                              ...
                              _confirmDelete(
                                ...
                                title: 'Xóa dịch vụ',
                                onConfirm: () =>
                                    session.removeService(service.id),
                              );
                            }
                          },
                          itemBuilder: (_) => const [
                    ...
                    return Card(
                      child: ListTile(
                        ...
                        trailing: PopupMenuButton<String>(
                          onSelected: (value) {
                            if (value == 'edit') {
                              _editPackage(context, session, existing: package);
                            } else if (value == 'delete') {
                              _confirmDelete(
                                context,
...
class _PackageFormDialogState extends State<_PackageFormDialog> {
  ...
  void _submit() {
    ...
    Navigator.of(context).pop(
      MockPackage(
        id: widget.existing?.id ?? 'pkg_${DateTime.now().microsecondsSinceEpoch}',
        name: name,
  ...
  Widget build(BuildContext context) {
    return AlertDialog(
      title: Text(widget.existing == null ? 'Thêm gói dịch vụ' : 'Sửa gói dịch vụ'),
      content: SingleChildScrollView(
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
